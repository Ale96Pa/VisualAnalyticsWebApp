...
function drawParallelChart(visualElement, data){
    ...
    dimensions = d3.keys(data[0]).filter(function(d) { 
        return d !== "year" && d !== "country" && d !== "gdp_per_capita" && d !== "country-year" 
                && d !== "gdp_for_year" && d !== "suicides_no" && d !== "population"})
    ...
    for (i in dimensions) {
        ...
        if (name === "suicides/100k pop"){range = rangeSuic;}
    ...
    svg.selectAll("myAxis")
        ...
        .each(function(d) {
            if (d === "year"){d3.select(this).call(d3.axisLeft().scale(y[d]));}
            ...
            if (d === "suicides/100k pop"){d3.select(this).call(d3.axisLeft()
                                            .scale(y[d])
                                            //.ticks(10)
